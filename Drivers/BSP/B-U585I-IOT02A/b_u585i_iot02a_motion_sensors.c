...
int32_t BSP_Get_Event_Status(uint32_t Instance, uint32_t Function, BSP_MOTION_SENSOR_Status_t *Status)
{
	  ...
	  else
	  {
	    ...
	    if (Motion_Sensor_FuncDrv[Instance][index]->Get_Event_Status(Motion_Sensor_CompObj[Instance], (int32_t *) Status) < 0)
	    {
...
static int32_t ISM330DHCX_Probe(uint32_t Functions)
{
  ...
  else
  {
    ...
    {
#if 1
#define DOUBLE_TAP_DETECTION_INT_PIN   ISM330DHCX_INT1_PIN
      /* Enable double-tap detection */
      if(ISM330DHCX_ACC_Enable_Double_Tap_Detection(&ISM330DHCX_Obj, DOUBLE_TAP_DETECTION_INT_PIN) != ISM330DHCX_OK)
      {
        status = BSP_ERROR_COMPONENT_FAILURE;
      }
#endif
